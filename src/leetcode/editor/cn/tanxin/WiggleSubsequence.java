

package leetcode.editor.cn.tanxin;
//如果连续数字之间的差严格地在正数和负数之间交替，则数字序列称为 摆动序列 。第一个差（如果存在的话）可能是正数或负数。仅有一个元素或者含两个不等元素的序列也
//视作摆动序列。 
//
// 
// 
// 例如， [1, 7, 4, 9, 2, 5] 是一个 摆动序列 ，因为差值 (6, -3, 5, -7, 3) 是正负交替出现的。 
// 
// 相反，[1, 4, 7, 2, 5] 和 [1, 7, 4, 5, 5] 不是摆动序列，第一个序列是因为它的前两个差值都是正数，第二个序列是因为它的最后一
//个差值为零。 
// 
//
// 子序列 可以通过从原始序列中删除一些（也可以不删除）元素来获得，剩下的元素保持其原始顺序。 
//
// 给你一个整数数组 nums ，返回 nums 中作为 摆动序列 的 最长子序列的长度 。 
//
// 
//
// 示例 1： 
//
// 
//输入：nums = [1,7,4,9,2,5]
//输出：6
//解释：整个序列均为摆动序列，各元素之间的差值为 (6, -3, 5, -7, 3) 。
// 
//
// 示例 2： 
//
// 
//输入：nums = [1,17,5,10,13,15,10,5,16,8]
//输出：7
//解释：这个序列包含几个长度为 7 摆动序列。
//其中一个是 [1, 17, 10, 13, 10, 16, 8] ，各元素之间的差值为 (16, -7, 3, -3, 6, -8) 。
// 
//
// 示例 3： 
//
// 
//输入：nums = [1,2,3,4,5,6,7,8,9]
//输出：2
// 
//
// 
//
// 提示： 
//
// 
// 1 <= nums.length <= 1000 
// 0 <= nums[i] <= 1000 
// 
//
// 
//
// 进阶：你能否用 O(n) 时间复杂度完成此题? 
// Related Topics 贪心 数组 动态规划 👍 641 👎 0

class P376_WiggleSubsequence{
    public static void main(String[] args) {
        //测试代码
        Solution solution = new P376_WiggleSubsequence().new Solution();
        int[] n = new int[]{1,2,5,2,2,5,5,2,6,6,1,1,0,9};
        System.out.println(solution.wiggleMaxLength(n));
    }
//力扣代码
	//leetcode submit region begin(Prohibit modification and deletion)
class Solution {
    // 可以当作回溯的组合吗？不可以
    // 摆动序列，即前后元素呈现+-+- / -+-+
    // 当前题是相当于从数组中，按原来排列顺序挑选元素出来
    // 如何找局部最优：如果遇到单调坡度，则只保留首尾 => 两个局部峰值
    // => 局部峰值越多，则摆动序列越长 => 合理
    // 用两个数表示前一对差值、当前一对差值；
    // 两对差值同时满足摇摆序列条件，则说明长度+1，更新前1对的差值
    public int wiggleMaxLength(int[] nums) {
        int preD = 0;   // 前一对差值
        int curD;   // 当前一对差值
        int result=1;   // 因为单个元素也算是摆动序列，所以默认算1
        for (int i = 1; i < nums.length; i++) {
            curD=nums[i]-nums[i-1];
            // 符合摆动序列
            // 为什么preD 可以=0？ 0的情况是为了照顾有相同元素情况
            if ((curD > 0 && preD<=0) || (curD < 0 && preD >= 0)){
                // 更新最大序列长度，和要对比的
                result++;
                preD=curD;
            }
        }
        return result;
    }

}
//leetcode submit region end(Prohibit modification and deletion)

}
